...
CMacIncrementalWithCopyStep::~CMacIncrementalWithCopyStep()
	{
	}
...
CMacIncrementalWithCopyStep::CMacIncrementalWithCopyStep()
	{
	SetTestStepName(KMacIncrementalWithCopyStep);
	}


TVerdict CMacIncrementalWithCopyStep::doTestStepPreambleL()
	{
	return EPass;
	}


TVerdict CMacIncrementalWithCopyStep::doTestStepL()
	{
	//Assume faliure, unless all is successful
	SetTestStepResult(EFail);
	
	INFO_PRINTF1(_L("*** Mac - Incremental with Copy ***"));
	...
	do 
		...
		if(readPosition == readIncrement)
			...
			macStr.Set(macCopyImpl->FinalL(*sourceData));
			CleanupStack::PopAndDestroy(sourceData);
		...
		//object hasn't already been copied
			...
			if(macCopied == EFalse)
				...
				{
				macCopyImpl->UpdateL(*sourceData);
	...
	if (*macResult == expectedMac)
		{
		INFO_PRINTF1(_L("*** Mac - Incremental with Copy : PASS ***"));
	...
	CleanupStack::PopAndDestroy(9, keyStr);	// keyStr, keyParams, key, macImpl, &fsSession, &sourceFile, macCopyImpl, macData, macResult
	INFO_PRINTF2(_L("HEAP CELLS: %d"), User::CountAllocCells());
	return TestStepResult();
	}


TVerdict CMacIncrementalWithCopyStep::doTestStepPostambleL()
	{
	return TestStepResult();
	}
